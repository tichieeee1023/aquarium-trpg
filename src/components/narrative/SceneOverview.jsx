...
const descriptions = {
  ...
  STAGE_4_MALL: '새벽 2시의 셔터 내린 대합실이다. 화강암 바닥은 차갑고 진짜 인간의 건축물이다.\n하지만 쇼윈도 속 목 없는 마네킹들이 고개를 꺾은 채 나를 내려다본다. 제어실 방화문 열쇠를 찾아야 한다.',
...
export default function SceneOverview({ stage }) {
  const [expanded, setExpanded] = useState(() => window.matchMedia('(min-width: 1401px) and (min-height: 801px)').matches);
  useEffect(() => {
    const media = window.matchMedia('(min-width: 1401px) and (min-height: 801px)');
  ...
  return (
    <details className="scene-overview" open={expanded} onToggle={(event) => setExpanded(event.currentTarget.open)}>
